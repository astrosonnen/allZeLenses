...
parnames = ['mhalo', 'alpha', 'mstar', 'lcvir', 'timedelay']
...
for i in range(nlens):
    chainname = chaindir+'%s_lens_%02d_mstarmhaloprior.hdf5'%(mockname, i)
    print chainname
...
def like(mhalo_mu=mhalo_mu, mhalo_sig=mhalo_sig, mstar_mhalo=mstar_mhalo, mstar_mu=mstar_mu, mstar_sig=mstar_sig, \
    ...
    for i in range(nlens):
        ...
        mhmu_prior = chains[i]['mhalo_prior'][0] + chains[i]['mhalo_prior'][1] * (chains[i]['mstar'] - 11.5)

        mhalo_prior = 1./chains[i]['mhalo_prior'][2]*np.exp(-0.5*(chains[i]['mhalo'] - mhmu_prior)**2/chains[i]['mhalo_prior'][2]**2)
        ...
        interim_prior = mhalo_prior * mstar_prior * alpha_prior
...
f = open('%s_mstarmhaloprior_inference.dat'%mockname, 'w')
