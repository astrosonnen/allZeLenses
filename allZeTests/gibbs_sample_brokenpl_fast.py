...
for i in range(nsamp-1):
    ...
    if eff_sigma != eff_sigma:
        indices = np.arange(nlens)
        bad = dt_model[:, i] != dt_model[:, i]
        print indices[bad]
        print xA_model[:, i][bad]
        print xB_model[:, i][bad]
        df
    ...
    for j in range(nlens):
        ...
        for k in range(ngrid_gamma):
            ...
            if np.isfinite(model_lenses[j].images[0]):
                ...
                logp_gamma[k] += -0.5*(model_lenses[j].radmag_ratio - radmagrat_obs[j])**2/radmagrat_err[j]**2
    ...
    for j in range(nlens):
        ...
        found = False
        repeats = 0
        while not found and repeats < 100:
            F = np.random.rand(1) * norm
    ...
    for j in range(nlens):
        print '%d %4.3f %4.3f %4.3f %4.3f'%(j, rein[j, i+1], gamma[j, i+1], beta[j, i+1], s2[j, i+1]**0.5)

    if (i+1) % save_every == 0:
        chain_file = h5py.File('tmp_%s_%dlenses_brokenpl_gibbs_sample_fast.hdf5'%(mockname, nlens), 'w')
...
chain_file = h5py.File('%s_%dlenses_brokenpl_gibbs_sample_fast.hdf5'%(mockname, nlens), 'w')
chain_file.create_dataset('invH0', data=invH0)
chain_file.create_dataset('mu_gamma', data=mu_gamma)
chain_file.create_dataset('sig_gamma', data=sig_gamma)
chain_file.create_dataset('mu_beta', data=mu_beta)
