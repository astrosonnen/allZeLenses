...
f = open('simple_reality_100lenses_inference.dat', 'r')
chain = pickle.load(f)
f.close()

f = open('simple_reality_100lenses_Nind1e4_inference.dat', 'r')
shortchain = pickle.load(f)
f.close()

f = open('simple_reality_100lenses.dat', 'r')
...
for par in chain:
    if par != 'logp':
	cp.append({'data': [shortchain[par], chain[par]], 'label': labels[par], 'value': mock['truth'][par], 'ticks': ticks[par]})

cornerplot(cp, color='c')
pylab.savefig('short_vs_long_interim_chains_cp.png')
pylab.show()
