...
for i in range(nlens):
    chainname = chaindir+'%s_lens_%02d_05fcaust.hdf5'%(mockname, i)
    print chainname
...
pars = [mhalo_mu, mhalo_sig, cvir_mu, cvir_sig, mstar_mhalo, mstar_mu, mstar_sig, aimf_mu, aimf_sig, H0]
...
def like(mhalo_mu=mhalo_mu, mhalo_sig=mhalo_sig, cvir_mu=cvir_mu, cvir_sig=cvir_sig, mstar_mhalo=mstar_mhalo, mstar_mu=mstar_mu, mstar_sig=mstar_sig, \
    ...
    for i in range(nlens):
        ...
        term = (mh_term*ms_term*a_term*dt_term*c_term/interim_prior).mean()
        totlike += np.log(term)
...
f = open('%s_05fcaust_inference.dat'%mockname, 'w')
