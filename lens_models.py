import numpy as np
from mass_profiles import sersic as sersic_profile, NFW as NFW_profile, deV_mlgrad as deV_mlgrad_profile
...
class broken_alpha_powerlaw: #spherical power-law with a break at the Einstein radius
    ...
    def kappa(self, x):
        return 2.**(self.beta - 1.) * ((3.-self.gamma)*(x/self.rein)**(1.-self.gamma)*(1.+x/self.rein)**(-self.beta) - self.beta * (x/self.rein)**(2.-self.gamma)*(1.+x/self.rein)**(-1.-self.beta))
