...
lens = lens_models.NfwGradDev(zd=0.3, zs=1., mstar=10.**mstar, mhalo=10.**mhalo, beta=beta_star, reff_phys=reff, delta_halo=200.)
...
f = open('/gdrive/projects/allZeLenses/mass_profiles/deV_mlgrad_re2_s2_spline.dat', 'r')
...
s2_bulge = lens.mstar * splev(lens.beta, deV_mlgrad_s2_spline)/reff
...
lens.psi3 = psi3_true
lens.mdme = mdme_true
lens.sigma = sigma_true

f = open('mock_mlgrad_for_example_truth.dat', 'w')
pickle.dump(lens, f)
f.close()
...
model_lens = lens_models.gNfwDev(zd=lens.zd, zs=lens.zs, mstar=lens.mstar, mdme=mdme_true, reff_phys=lens.reff_phys, beta=1., rs_phys=10.*lens.reff_phys, delta_halo=200., source=lens.source)
...
f = open('gnfwdev_to_mlgrad_wdyn_sample.dat', 'w')
pickle.dump(chain, f)
f.close()
...
pylab.savefig('gnfwdev_to_mlgrad_wdyn_cp.png')
#pylab.show()
