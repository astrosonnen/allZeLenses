...
chain_file = h5py.File('%s_powerlaw_chains.hdf5'%mockname, 'r')
...
for i in range(nlens):
    ...
    dt_model = chain_file['lens_%02d'%i]['timedelay'].value.copy()
...
#ax1.errorbar(taylor_fit/taylor_true, ind_H0, yerr=H0_err, xerr=taylor_err/taylor_true, fmt='+', color='b')
...
pylab.text(-0.08, 15., 'model: power-law', fontsize=fsize)
pylab.text(-0.08, 5., 'constraints: image positions, $r_\mu$', fontsize=fsize)

fig = pylab.gcf()
fig.set_size_inches(6, 8)
pylab.savefig('individual_H0.eps')
#pylab.show()
