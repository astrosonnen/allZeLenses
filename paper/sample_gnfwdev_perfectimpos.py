...
for i in range(nlens):
    ...
    chainname = chaindir+'%s_lens_%02d_gnfwdev_perfect_impos.hdf5'%(mockname, i)
    ...
    if not os.path.isfile(chainname):
        ...
        # calculates true sigma (because I forgot to save it with the mock)
        m200tomrs = (np.log(2.) - 0.5)/(np.log(1. + lens.cvir) - lens.cvir/(1. + lens.cvir))
        s2_halo = lens.mhalo * m200tomrs*splev(lens.rs/lens.reff, nfw_re2_s2_spline)/lens.reff_phys
        s2_bulge = lens.mstar * deV_re2_s2 / lens.reff_phys
        ...
        def logp(sig=sigma, rmu=radmagratio):
            return - 0.5*(sig - sigma_obs)**2/sigma_err**2 - 0.5*(rmu - lens.radmag_ratio)**2/radmagrat_err**2
