...
def fit_nfwdev_fixsbound(lens, nstep=15000, burnin=5000, thin=1, mhalo_prior=(13., 0.5), mstar_prior=(11.5, 0.5), alpha_prior=(0., 0.2), cvir_prior=(0.877, 0.3)):
...
def fit_cored_powerlaw_imposonly(lens, nstep=15000, gamma_prior=(2., 0.2), burnin=5000, thin=1):
    ...
    model_lens = lens_models.cored_powerlaw(zd=lens.zd, rc=1e-4, zs=lens.zs, rein=rein_guess, gamma=2., images=lens.images, source=lens.source)
    model_lens.make_grids(err=imerr, nsig=5.)

    gamma_par = pymc.Normal('gamma', mu=gamma_prior[0], tau=1./gamma_prior[1]**2, value=2.)
    rein_par = pymc.Uniform('rein', lower=0.2*rein_guess, upper=3.*rein_guess, value=rein_guess)

    @pymc.deterministic()
    def caustic(rein=rein_par, gamma=gamma_par):
        model_lens.rein = rein
        model_lens.gamma = gamma
    ...
    s2_par = pymc.Uniform('s2', lower=0., upper=caustic**2, value=(xA - model_lens.alpha(xA))**2)
    ...
    M = pymc.MCMC(pars + [images, caustic, timedelay])
    M.use_step_method(pymc.AdaptiveMetropolis, pars)
    M.sample(nstep, burnin, thin=thin)

    outdic = {'gamma': M.trace('gamma')[:], 'rein': M.trace('rein')[:], 'source': M.trace('s2')[:]**0.5, \
              'timedelay': M.trace('timedelay')[:].flatten(), 'caustic': M.trace('caustic')[:], \
              'images': M.trace('images')[:], 'gamma_prior': gamma_prior}
