...
def nfw_mhalo_given_mstar(nlens=1000, mstar_mu=11.5, mstar_sig=0.3, mhalo_mu=13.0, mhalo_sig=0.4, mhalo_beta=1.5, \
                          cvir_sig=0.1, cvir_mu=0.877, cvir_beta=-0.094, aimf_mu=-0.1, aimf_sig=0.05, logreff_mu=0.46, \
    ...
    # distribution of stellar masses: Gaussian
    mstars = mstar_mu + np.random.normal(0., mstar_sig, nlens)

    # distribution of halo masses: power-law dependence on stellar mass + Gaussian scatter
    mhalos = mhalo_mu + mhalo_beta * (mstars - 11.5) + np.random.normal(0., mhalo_sig, nlens)
    ...
    hyperpars = {'h': h, 'mstar_mu': mstar_mu, 'mstar_sig': mstar_sig, 'mhalo_mu': mhalo_mu, 'mhalo_sig': mhalo_sig,  \
                 'mhalo_beta': mhalo_beta, 'cvir_mu': cvir_mu, 'cvir_beta': cvir_beta, 'cvir_sig': cvir_sig, \
